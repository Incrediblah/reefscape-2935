...
public class OdometryCmd extends SequentialCommandGroup {
    ...
    public OdometryCmd(DriveSubsystem driveSubsystem, String selectedPath) {
        ...
        Trajectory RobotRightFromlineToReef = TrajectoryGenerator.generateTrajectory(
            ...
            new Pose2d(0.43, 1.41, new Rotation2d(0)),
            forwardConfig
        );

        Trajectory RobotLeftFromlineToReef = TrajectoryGenerator.generateTrajectory(
          new Pose2d(0, 0, new Rotation2d(0)),
          List.of(new Translation2d(0.09, -0.01), new Translation2d(0.33, 0.04), new Translation2d(1, 0), new Translation2d(1.15, 0.24)),
          new Pose2d(1.41, 0.43, new Rotation2d(0)),
          reverseConfig
        );

        Trajectory robotStartRightFirstPiece = TrajectoryGenerator.generateTrajectory(
          new Pose2d(0, 0, new Rotation2d(0)),
        ...
        // SwerveControllerCommands with overridden isFinished() method
        SwerveControllerCommand RobotRightFromlineToReefCmd = new SwerveControllerCommand(
            ...
            public boolean isFinished() {
                return super.isFinished() || hasReachedFinalPose(RobotRightFromlineToReef);
            }
        };
        ...
        SwerveControllerCommand RobotLeftFromlineToReefCmd = new SwerveControllerCommand(
            ...
            @Override
            public boolean isFinished() {
                return super.isFinished() || hasReachedFinalPose(RobotLeftFromlineToReef);
            }
        };

        SwerveControllerCommand robotStartRightFirstPieceCmd = new SwerveControllerCommand(
            robotStartRightFirstPiece,
            DRIVE_SUBSYSTEM::getPose,
            DriveConstants.kDriveKinematics,
            xController,
            yController,
            ...
            @Override
            public boolean isFinished() {
                return super.isFinished() || hasReachedFinalPose(robotStartRightFirstPiece);
            }
        };

        // Select the correct path based on the input
        if (selectedPath.equals("right")) {
            path = RobotRightFromlineToReefCmd;
        } else if (selectedPath.equals("left")) {
            path = RobotLeftFromlineToReefCmd;
        } else if (selectedPath.equals("robotStartRightFirstPiece")) {
