...
public class AlignXandYWithPhoton extends Command {
    ...
    /** Creates a new alignmentCommand. */
    public AlignXandYWithPhoton(DriveSubsystem drive, PhotonSubsystem photon, boolean end, 
                                double targetOffsetX, double targetOffsetY, double toleranceX, double toleranceY) { // NEW: Accepts a locked heading from `TurnToAprilTagCommand`
        this.DRIVE_SUBSYSTEM = drive; 
    ...
    public void initialize() {
      ...
      //   int detectedTag = VISION_SUBSYSTEM.getLeftAprilTagID(); 
        if (detectedTag != -1) { // Ensure a valid tag was detected
            ...
            targetAngle = PHOTON_SUBSYSTEM.getReefAngleForTag(detectedTag, AutoConstants.autoMode); // Get pre-defined reef angle
        } else {
    ...
    public void execute() {
        if(PHOTON_SUBSYSTEM.hasTargets()){
            measuredValueY = PHOTON_SUBSYSTEM.getPitchToBestTarget();
            measuredValueX = PHOTON_SUBSYSTEM.getYawToBestTarget();  
