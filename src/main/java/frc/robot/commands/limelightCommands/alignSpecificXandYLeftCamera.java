...
public class alignSpecificXandYLeftCamera extends Command {
    ...
    public alignSpecificXandYLeftCamera(DriveSubsystem drive, VisionSubsystem vision, int pipeline, boolean end, 
                                double targetOffsetX, double targetOffsetY, double toleranceX, double toleranceY, int id) { // NEW: Accepts a locked heading from `TurnToAprilTagCommand`
    ...
    public void execute() {
        if(VISION_SUBSYSTEM.limelightLeftTargetSeen()){
            measuredValueY = VISION_SUBSYSTEM.getLeftSpecificTagTy(target);
            measuredValueX = VISION_SUBSYSTEM.getLeftSpecificTagTx(target); 
